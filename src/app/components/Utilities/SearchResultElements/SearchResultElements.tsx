...
export default function SearchResultElements(
  ...
  if (results["message"] == "No more relevant products") {
    response = [
      <div>
        Looks like we have no relevant products, hang on to us soon we will be
    ...
    for (let result of keys) {
      ...
      response.push(
        <div
          ...
          onClick={() => {
            setClicks(clicks + 1);
            services.clicked(access_token, product["id"], product["tags"]);
            setProductDetails(product);
          }}
          key={result}
        >
          {product != null ? (
            Utilities.SearchResultElement(
              product["id"],
              product["brand"],
