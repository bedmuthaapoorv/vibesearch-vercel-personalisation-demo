...
export default function SearchResults() {
  ...
  let [personalisation, setPersonalisation]=useState<number>(0)
  let [personalisationAvailaible, setPersonalisationAvailaible]=useState<boolean>(false)
  ...
  let [search_id, setSearchId]=useState<any>(null)
  useEffect(() => {
    window.scroll(0, 0);
}, [pathname]);
  useEffect(() => {
    ...
    services.getAccessToken(setAccessData).then((accessData) => {
      ...
      if (accessData && accessData["data"] && accessData["data"]["session"]) {
        ...
        setPersonalisation(searchParams.get("personalisation")=='1'? 1: 0);
  ...
  useEffect(() => {
    if(clicks==5){
      setPersonalisationAvailaible(true)
    }
  }
,[clicks])
